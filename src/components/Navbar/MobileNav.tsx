  ...
  return (
    <>
      ...
      >
        ...
        <motion.ul className={
          `flex-row gap-4 items-center fixed bottom-4 left-4 right-4 ${open ? 'flex h-fit' : 'h-0 pointer-events-none opacity-0'} md:hidden`
        }>
          {
            SOCIAL_MEDIA && SOCIAL_MEDIA.map((item: any) => (
              <motion.li
                ...
                whileFocus={{ scaleY: 1.1 }}
                whileTap={{ scaleY: 0.9 }}
                className='md:hidden border-b border-zinc-400 w-full pb-2 text-xl'
                onClick={() => setOpen(!open)}
              >
                <Link
                href={item.href} className='text-stone-500 font-normal' >
